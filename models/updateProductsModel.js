...
async function guardarImagen(archivo, rutaImagenAnterior) {
    return new Promise(async (resolve, reject) => {
        ...
        const nombreArchivo = `imgimgPlatillo_${Date.now()}${extension}`;
...
async function actualizarArticulo(id, nombrePlatillo, categoria, descripcionPlatillo, precio, imagenUrlPlatillo) {
    const sql = 'UPDATE platillos SET nombrePlatillo=?, categoria=?, descripcionPlatillo=?, precio=?, imagenUrlPlatillo=? WHERE id=?';
    const values = [nombrePlatillo, categoria, descripcionPlatillo, precio, imagenUrlPlatillo, id];

    return new Promise((resolve, reject) => {
        db.connection.query(sql, values, async (queryErr, results) => {
            if (queryErr) {
                console.error('Error al actualizar el artículo: ' + queryErr.message);
                reject(false);
            } else {
...
async function obtenerRutaImagenAnterior(id) {
    const sql = 'SELECT imagenUrlPlatillo FROM platillos WHERE id = ?';
    const values = [id];
    ...
    return new Promise((resolve, reject) => {
        db.connection.query(sql, values, (queryErr, results) => {
            if (queryErr) {
                ...
                if (results.length > 0) {
                    resolve(results[0].imagenUrlPlatillo);
                } else {
                    resolve(null);
                }
            }
